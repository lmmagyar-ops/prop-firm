...
// computeWinRate was removed from position-utils during a prior refactor.
// Inlined here so the behavioral test continues to verify the algorithm.
function computeWinRate(trades: { type: string; realizedPnL: string | null }[]): number | null {
    const sells = trades.filter(t => t.type === 'SELL' && t.realizedPnL !== null);
    if (sells.length === 0) return null;
    const wins = sells.filter(t => parseFloat(t.realizedPnL!) > 0);
