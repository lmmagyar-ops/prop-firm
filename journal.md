...
**1. CRITICAL — Year-rollover bug in `isStaleMarketQuestion`**
...
**2. MEDIUM — `isMultiOutcome` undefined at runtime**
...
**3. LOW — Test label mismatch**
`tests/lib/stale-market-filter.test.ts:43` — says `"returns false for January"` but asserts `toBe(true)`.
